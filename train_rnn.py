...
from utils.datasets import BatchDatasetV2
...
if __name__ == '__main__':
    ...
    for fold in folds:
        ...
        X_train_1 = np.load(f'./data/fold_{fold}_train_norm_left.npy')
        X_train_2 = np.load(f'./data/fold_{fold}_train_norm_right.npy')
        X_valid_1 = np.load(f'./data/fold_{fold}_valid_norm_left.npy')
        X_valid_2 = np.load(f'./data/fold_{fold}_valid_norm_right.npy')
        ...
        train_dataset = BatchDatasetV2(
            X_train_1, X_train_2, y_train, t_train, augment=True,
            shuffle=True, std_x=std_x, std_y=std_y, weight=w_rank,
            sample=sample, scale=scale,
        )

        valid_dataset = BatchDatasetV2(
            X_valid_1, X_valid_2, y_valid, t_valid, augment=False,
            shuffle=False, std_x=0.0, std_y=0.0, weight=w_rank,
            sample=1.0, scale=scale,
        )
